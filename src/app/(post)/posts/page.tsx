...
const Posts = async ({ searchParams }: { searchParams: SearchParams }) => {
	...
	return (
		<div
			className={cn(
				'relative min-h-screen bg-stone-900 leading-relaxed text-stone-400 antialiased selection:bg-accent-400 selection:text-stone-900',
			)}
		>
			<div
				className={cn(
					'mx-auto min-h-screen max-w-screen-xl px-6 py-12 font-sans md:px-12 md:py-20 lg:px-24',
				)}
			>
						...
						{posts.map((post, index) => {
							...
							return (
								<article
									key={post.slug}
									className={cn(
										'group relative flex flex-col justify-end overflow-hidden rounded-2xl border border-stone-800/50 bg-stone-900 transition-all hover:scale-[1.01] hover:shadow-2xl hover:shadow-accent-500/20',
								...
								>
									<div className={cn('absolute inset-0 z-0 h-full w-full')}>
										<div
											className={cn(
												'h-full w-full bg-gradient-to-br from-stone-800 via-stone-900 to-stone-950 opacity-60 transition-transform duration-700 group-hover:scale-105',
											)}
										/>
										<div
											className={cn(
									...
									>
										<header className={cn('mb-2')}>
											...
											>
												{post.createdTime.format('YYYY-MM-DD')}
											</span>
										</header>
										...
										<div>
											...
											>
												...
												>
													...
													<span
														className={cn(
															'bg-gradient-to-r from-stone-100 to-stone-300 bg-clip-text text-transparent transition-all duration-300 group-hover:from-accent-200 group-hover:to-accent-500',
														)}
													>
														{post.title}
													</span>
										...
										<div
											className={cn(
												'mt-4 flex translate-y-2 transform items-center text-sm font-medium text-accent-400 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100',
											)}
										>
											Read Article{' '}
